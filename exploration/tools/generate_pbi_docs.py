...
def generate_pbi_documentation():
    """
    Generates the master Power BI documentation (pbi_advanced_measures.md).
    Includes the full metric inventory (16 metrics), logic for dynamic narratives, 
    data imputation awareness, navigation, and scatter chart animations.
    """
    # Path resolution: src/utils -> project_root -> docs/
    ...
    content = """# Power BI Master Measures & Design Blueprint
...
"""
    ...
    try:
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"[OK] PBI Documentation generated at: {file_path}")
    except Exception as e:
        print(f"[ERROR] Failed to generate PBI documentation: {e}")
